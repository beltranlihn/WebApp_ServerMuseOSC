 ...
 * <h3>EEG derived signals</h3>
 * <h4>Absolute band powers</h4>
 * The absolute band power for a given frequency range (for instance, alpha, i.e. 7.5-13Hz) is the logarithm of the
 * sum of the Power Spectral Density of the EEG data over that frequency range. They are provided for each of the
 * channels/electrode sites on Muse. Since it is a logarithm, some of the values will be negative
 * (i.e. when the absolute power is less than 1). They are given on a log scale, units are Bels.
 *
 * <h4>Relative band powers</h4>
 * The relative band powers are calculated by dividing the absolute linear-scale power in one band over the sum of
 * the absolute linear-scale powers in all bands. The linear-scale band power can be calculated from the log-scale
 * band power.
 * 
 * linear-scale band power = 10^ (log-scale band power)
 *
 ...
 *
 * <h4>Band power scores</h4>
 * The band session score is computed by comparing the current value of a band power to its history. This current value
 * is mapped to a score between 0 and 1 using a linear function that returns 0 if the current value is equal to or below
 * the 10th percentile of the distribution of band powers, and returns 1 if it's equal to or above the 90th percentile.
 * Linear scoring between 0 and 1 is done for any value between these two percentiles.
 *
 * Be advised that these scores are based on recent history and it will take a few seconds before having a stable distribution
 * to score the power against. The estimated distribution is continuously updated as long as the headband is on the head.
 * However, every time it's updated, the newest values are weighted to have more importance than the historical values.
 * This means that eventually old values will not be present anymore in the estimated distribution.
 * The half-life of the estimated distribution at any given point is around 10 seconds.
 *
 * The score will start being calculated as soon as the headband has established a good connection with the skin.
 * Whenever the headset loses connection with the head (as determined by the DRL/REF contact quality) the estimated
 * distributions are reset. This means that when the headband is removed, the session data from any previous user will be cleared.
 *
 */
