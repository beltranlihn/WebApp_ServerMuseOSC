...
public:
     ...
     * \param model the Muse model to which the conversion is applied.
     * \param ppg_value the raw PPG channel value.
     * \return the PPG value in microamps.
     * \exception SIGABRT
     */
    virtual double get_ppg_microamps(MuseModel model, double ppg_value) = 0;

    /**
     * Get the optics value from the packet.
     * Calling this function does not perform additional allocations and is preferable to using
     * values().
     * \if ANDROID_ONLY
     * \param channelNum the optics channel to retrieve. (ie. \link com.choosemuse.libmuse.Optics.OPTICS1\endlink)
     * \elseif IOS_ONLY
     * \param channelNum the ::IXNOptics channel to retrieve (ie. \link IXNOptics IXNOpticsOPTICS1\endlink)
     * \elseif WINDOWS_ONLY
     * \param channel_num the
     * \link ::interaxon::bridge::Optics OPTICS\endlink
     * channel to retrieve (ie. Optics::OPTICS1)
     * \else
     * MISSING_DOCUMENTATION
     * \endif
     * \return the value requested.
     * \exception SIGABRT
     * If this function is called on a packet type that is not type
     * \enumlink{MuseDataPacketType,OPTICS,IXNMuseDataPacketTypeOptics},
     * LibMuse will throw an exception. Use
     * \inlinelink{packetType(),::IXNMuseDataPacket::packetType(),packet_type()}
     *  to check the type before calling this function.
     */
